...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Execute the information extraction pass over SQLite data."
    )
    parser.add_argument(
        "--sqlite", type=Path, default=Path("./discord.db"), help="Path to the SQLite database."
    )
    parser.add_argument(
        "--window-size", type=int, default=8, help="Message window size to supply to the LLM."
    )
    parser.add_argument(
        "--max-windows",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum confidence to store a fact.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--timeout", type=float, default=600.0, help="HTTP client timeout (seconds)."
    )
    parser.add_argument("--api-key", help="Optional bearer token if llama-server requires auth.")
    parser.add_argument(
        "--resume", action="store_true", help="Resume the most recent paused IE run."
    )
    parser.add_argument(
        "--reset-progress",
        action="store_true",
